...
interface ShareButtonEnProps {
  className?: string;
}

const ShareButtonEn: React.FC<ShareButtonEnProps> = ({ className }) => {
    ...
    >
      ...
      {showMenu && (
        ...
        >
          ...
          <a href={`mailto:?subject=Resume Denis Dmitrievich Minishin - Design Engineer, Process Control Engineer&body=Denis Dmitrievich Minishin - Design Engineer, Process Control Engineer%0A%0Ahttps://${window.location.host}/en – Resume%0Ahttps://${window.location.host}/en#portfolio – Portfolio`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <EmailIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">E-mail</span>
          </a>
          <a href={`https://t.me/share/url?url=https://${window.location.host}/en&text=${RESUME_DATA_EN.name} – Design Engineer, Process Control Engineer`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <TelegramIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">Telegram</span>
          </a>
          <a href={`https://api.whatsapp.com/send?text=Resume Denis Dmitrievich Minishin – https://${window.location.host}/en`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <WhatsappIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">WhatsApp</span>
          </a>
          <a href={`viber://forward?text=https://${window.location.host}/en&text=Resume – ${RESUME_DATA_EN.name} | Design Engineer, Process Control Engineer`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <ViberIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">Viber</span>
          </a>
          <a href={`https://vk.com/share.php?url=https://${window.location.host}/en&title=Resume – ${RESUME_DATA_EN.name} | Design Engineer, Process Control Engineer`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <VkIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">VK</span>
          </a>
          <a href={`https://yandex.ru/chat?text=https://${window.location.host}/en%0AResume – ${RESUME_DATA_EN.name} | Design Engineer, Process Control Engineer#/forward`} target="_blank" className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <YandexMessengerIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">Yandex.Messenger</span>
          </a>
          <a onClick={copyToClipboard} className="flex items-center space-x-2 hover:bg-gray-700 px-2 py-1 rounded cursor-pointer">
            <CopyIcon className="w-5 h-5" />
            <span className="whitespace-nowrap text-base">Copy link</span>
          </a>
        </div>
      )}
      ...
      {showOverlay && (
        <div
          className={`fixed top-0 left-0 w-full h-full bg-black/35 z-20 transition-opacity duration-300 ${showMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'
            }`}
